    ...
    $checkUser = mysqli_query($conn, "SELECT *  FROM pin WHERE pin='$pin' and studentId = '$studId' and validity= '1' and sch = '$schoolId' ");
          if (mysqli_num_rows($checkUser) > 0) 
          {
           ...
           if (mysqli_num_rows($query)>0) 
           {
            ...
            if (isset($_SESSION['examTerm']) && !empty($_SESSION['examTerm']) && isset($_SESSION['examYear']) && !empty($_SESSION['examYear']) && isset($_SESSION['reg']) && !empty($_SESSION['reg']))
            {
            echo "
                <script>
           ...
           else
           {
            ...
            "
            <script> 
                  alert ('Some parameters are not correctly set');
         ...
         else
         {
          ...
          if (mysqli_affected_rows($conn)>0) 
          {
           ...
           if (mysqli_num_rows($query)>0) 
           {
            ...
            $_SESSION['examYear'] =$examYear;
            $_SESSION['reg'] =$adminNum; 
            if (isset($_SESSION['examTerm']) && isset($_SESSION['examYear']) && isset($_SESSION['reg']))
            {
              echo "
                <script>
                  window.location.href='myResult-Checker.php?StudentId=".($url_data)."'
...
require '../controller/parents_required.php'; 
   ...
   meta();
   siteTitle(mysite, " || ", " Parents" ." || " . $_SESSION['username']."'s portal");
  $pagename='dashboard';
    pageHeader();
   sideBar();
   
   $studId = $_SESSION['studId'];
   $studSch = $_SESSION['schoolId'];

   $getReg = mysqli_query($conn, "SELECT regNum FROM students WHERE id = '$studId'");
   if (mysqli_num_rows($getReg)>0) {
     while ($studReg = mysqli_fetch_array($getReg)) {
      $_SESSION['reg'] = $studReg['regNum'];
      $reg = $_SESSION['reg'];
     }
   }
