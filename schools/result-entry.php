...
session_start();
    require '../db/db_con.php';
     if (isset($_POST['saveResult'])) {
     	...
     	if (!empty($_POST['examYear']) && !empty($_POST['examTerm']) && !empty($_POST['studId']) && !empty($_POST['studClass']) && !empty($_POST['subject']) && is_array($_POST['examYear']) && is_array($_POST['examTerm']) && is_array($_POST['studId']) && is_array($_POST['studClass']) && is_array($_POST['subject']) && count($_POST['studId']) === count($_POST['examYear'])) {
     	...
     	$auth = mysqli_query($conn, "SELECT * FROM student_result WHERE  examYear='$f1' and examTerm='$f2' and studentId = '$f3' and subjectId='$f5'");
     	...
     	$query =mysqli_query($conn, "INSERT INTO `student_result`(`examYear`, `examTerm`, `studentId`, `classId`, `subjectId`, `cat`, `exam`, `total`, `grade`, `remark`, `position`) VALUES ('$f1', '$f2', '$f3', '$f4', '$f5', '$f6', '$f7', '$f8', '$f10', '$f11', '$f12')");
...
siteTitle(mysite, ' || ', '  Admin'.' || ' . $_SESSION['user']);
  ...
  <input type="hidden"  value="'.$_SESSION['subject'].'"  name="subject[]">
    ...
    <td> 
          <div class="form-group">
            ...
            <input type="number" class="form-control form-control-inline input-medium cat" id="cat" name="cat[]" min="0" max="40" onkeypress="return (event.charCode == 8 || event.charCode==0)? null: event.charCode >=48 && event.charCode <=57" required="">
            </td>
            
            <td>
            <input type="number" class="form-control form-control-inline input-medium examScore" id="examScore" name="examScore[]" min="0" max="60"  onkeypress="return (event.charCode == 8 || event.charCode==0)? null: event.charCode >=48 && event.charCode <=57" required="">
            </td>
            
             <td>
            <input type="text" class="form-control form-control-inline input-medium totScore" id="totScore" name="totScore[]"  readonly="" min="0" max="100">
            </td>
            <td>
  ...
  <td colspan="7">
  <a  data-toggle="modal" class="btn btn-theme btn-sm pull-right" href="result-upload.php#myModal">Save Result&nbsp;<i class="fa fa-save"></i> </a><br><br>  
  </td>
  </tbody>
  </table>
  ...
  ';
 	...
 	window.location.href='result-upload.php';</script>
